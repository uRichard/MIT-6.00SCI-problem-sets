...
class League:
    ...
    def rank_team(self,team):
        ...
        team: a tuple. ("Arsenal",10,6) where 5 = goals scored, 15 = goals conceded.and over 3 wins.
       ...
       # wins_two_goals = self.get_wins_over_two_goals(team[0],match_number)
       # wins_one_goal = self.get_wins_of_one_goal(team[0],match_number)
       # draws_of_zero = self.get_draws_zero_zero(team[0],match_number)
    ...
    def get_wins_over_two_goals(self,team_name, match_number):
        """
        Compute and return the wins of a team where a team scored over 2 goals.
        Assume the best team makes a win two or graeter than two goals.
        Teams with more wins are the best.
        teams a list with elements that have a team and its results.
        e.g [("Arsenal","Liverpool",2,3)] use compute_games_played.
        return: list {"Arsenal":["W","D","L"}
        Assumptions: A team with 3 wins or more and has conceded less than min
        minimum threshold is the best.
        We cannot work without this assumption. 
        We will use this method in ranking a team.
        If two teams with 3 or more such wins play each other,
        bet over 1.5. This is more exact than both teams to score.

        """
        results = {team_name:[]}
        matches = self.computes_games_played(team_name, match_number)
        for match in matches:
            home_team = match[2]
            home_team_score = int(match[4])
            away_team = match[3]
            away_team_score = int(match[5])
            if home_team == team_name and (home_team_score >= 2 and home_team_score >= away_team_score): #if team is playing at home.
                results[team_name].append("W")
            elif away_team == team_name and(away_team_score >= 2 and away_team_score >= home_team_score):
                results[team_name].append("W")

        return results

    def get_wins_of_one_goal(self,team_name, match_number):
        """Computes and returns wins of 1 goal.
        This will be used in ranking a team.
        """
        results = {team_name:[]}
        matches = self.computes_games_played(team_name, match_number)
        for match in matches:
            ...
            away_team = match[3]
            away_team_score = int(match[5])
            if home_team == team_name and (home_team_score == 1 and home_team_score >= away_team_score): #if team is playing at home.
                results[team_name].append("W")
            elif away_team == team_name and(away_team_score == 1 and away_team_score >= home_team_score):
                results[team_name].append("W")

        return results

    def get_draws_zero_zero(self,team_name,match_number):
        """Computes and returns the draws where its zero zero.
        """
        results = {team_name:[]}
        matches = self.computes_games_played(team_name, match_number)
        for match in matches:
            ...
            away_team = match[3]
            away_team_score = int(match[5])
            if home_team == team_name and (home_team_score == 0 and home_team_score == away_team_score): #if team is playing at home.
                results[team_name].append("Draw of zero.")
            elif away_team == team_name and(away_team_score == 1 and away_team_score == home_team_score):
                results[team_name].append("Draw of zero.")
...
def menu(teams, league,match_number):
    ...
    while True:
        ...
        if home_team == "q" or away_team == "q":
           ...
           home_team_win_over_two_goals = league.get_wins_over_two_goals(home_result[0], match_number)
           print(home_result[0]+" Wins of over 2 goals: ",home_team_win_over_two_goals)
           home_team_wins_one_goal = league.get_wins_of_one_goal(home_result[0],match_number)
           print(home_result[0]+" Wins of One goal: ",home_team_wins_one_goal)
           home_zero_zero_draws = league.get_draws_zero_zero(home_result[0],match_number)
           print(home_result[0]+" Zero Zero Draws: ", home_zero_zero_draws)
           print("\n")
           ...
           away_team_win_over_two_goals = league.get_wins_over_two_goals(away_result[0], match_number)
           print("Wins of over 2 goals: ",away_team_win_over_two_goals)
           away_team_wins_one_goal = league.get_wins_of_one_goal(away_result[0],match_number)
           print("Wins of One goal: ",away_team_wins_one_goal)
           away_zero_zero_draws = league.get_draws_zero_zero(away_result[0],match_number)
           print("Zero Zero Draws: ", away_zero_zero_draws)
...
def main():
    ...
    #SCOTLAND
    #filename = "SCOTLAND_PREMIER_LEAGUE.csv" #scotland premier league.
    #filename = "SCOTLAND_DIVISION1.csv" #scotland division one.
    #filename = "SCOTLAND_DIVISION2.csv" #scotland division 2.
    #filename = "SCOTLAND_LEAGUE2.csv" #scotland league2
    
    #NETHERLANDS
    #filename = "NETHERLANDS_EREDIVISIE.csv"
    ...
    """
    team_name = "Halifax"
    results = league.get_wins_over_two_goals(team_name, match_number)
    print("Wins of Man City over Two Goals: ",results)
    
    wins = league.get_wins_of_one_goal(team_name,match_number)
    print("Wins of One goal: ",wins)
    
    zer_zero_draws = league.get_draws_zero_zero(team_name,match_number)
    print("Zero Zero Draws: ", zer_zero_draws)
    """
    ...
    print(myBet)
    print("Teams with more than three wins are the best. Bet Win or over 1.5 or both teams to score.")
