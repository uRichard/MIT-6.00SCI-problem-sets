...
class League:
    ...
    def grade_team(self,team):
        ...
        #max_goals_scored = goal_threshold - asummed_conceded_goals #goals in 5 match days.
        
       
        max_goals_scored = self.compute_goal_threshold() 
        #max_goals_scored = max_goals_scored - asummed_conceded_goals
        #goal scoring range.
        near_max_goals_scored = round( (3/4) * max_goals_scored )
        average_goals_scored = round( (1/2) * max_goals_scored )
        min_goals_scored = round( (1/4) * max_goals_scored )
        
        #goals conceded range.
        near_max_goals_conceded = round( (3/4) * max_goals_scored )
        average_goals_conceded = round( (1/2) * max_goals_scored )
        min_goals_conceded = round( (1/4) * goal_threshold)
        ...
        if (goals_scored >= max_goals_scored and goals_conceded <= min_goals_conceded) or (goals_scored >= near_max_goals_scored and goals_conceded <= min_goals_conceded):
            grade = grade + "A"

        #case 2: average teams. good for both teams to score.
        elif (goals_scored >= max_goals_scored and goals_conceded >= average_goals_conceded) or (goals_scored >= near_max_goals_scored and goals_conceded >= average_goals_conceded) or (goals_scored >= average_goals_scored and goals_conceded >= average_goals_conceded ):
            grade = grade + "B"
        
        #case 3: worst teams.
        elif (goals_scored <= min_goals_scored and goals_conceded >= max_goals_scored) or (goals_scored <= min_goals_scored and goals_conceded >= near_max_goals_conceded)or (goals_scored <= min_goals_scored and goals_conceded >= average_goals_conceded):
            grade = grade + "C"
    ...
    def both_teams_to_score(self,match_number):
        ...
        average_total_goals = round( (1/2) * max_goal_threshold)
    ...
    def best_teams_to_win(self,match_number):
        ...
        average_total_goals = round( (1/2) * max_goal_threshold)
        min_goals_goal_threshold =  round( (1/4) * max_goal_threshold)
        teams = self.get_league_teams()
        teams_data = self.compute_goals_scored_goals_conceded(teams,match_number)
        ...
        for result in teams_data:
            goals_scored = int(result[1])
            goals_conceded = int(result[2])
            if (goals_conceded <= min_goals_goal_threshold and goals_scored >= near_max_goal_threshold) or (goals_conceded <= min_goals_goal_threshold and goals_scored > average_total_goals):
                results.append(result)
    ...
    def second_best_teams(self, match_number):
        ...
        for result in teams_data:
            goals_scored = int(result[1])
            goals_conceded = int(result[2])
            if (goals_conceded <= min_goals_goal_threshold) and (goals_scored >= average_total_goals and goals_scored < near_max_goal_threshold):
                results.append(result)
    ...
    def worst_teams(self,match_number):
        ...
        for result in teams_data:
            goals_scored = int(result[1])
            goals_conceded = int(result[2])
            if (goals_scored <= min_goals_goal_threshold) and (goals_conceded >= average_total_goals):
                results.append(result)
...
def menu(teams, league,match_number):
    ...
    while True:
        ...
        if home_team == "q" or away_team == "q":
           ...
           print("Grade:")
           grade_teams = grade_all_teams(teams,league,match_number)
...
def bet3(results):
    """
    Uses the list of graded teams from grade_all_teams() to make a bet.
    results: list. e.g [A: [("Arsenla","comment")], B: [("Everton","comment"), ]
    Team with grade A must WIN regardles which it is playing against.
    Team with grade B scores more than average and concedes more than average.
    Team with grade C is worst team in league.
    
    Bet: Both teams to score.
        if both teams have grade B.
    Bet: Team to win.
       If team has grade A, bet Win for A.
       
    """
    bet = ""
    results = results[:]
    for result in results.keys():
        items = results[result] # retuns list
        pass
    return None
...
def main():
    ...
    #filename = "SCOTLAND_LEAGUE1.csv" #scotland division 2. rename file
    ...
    print("Threshold goals for a team in 5 matches.")
    print("Max Threshold goals scored: ",max_goals_scored)
    print("Near max Threshold goals scored: ",near_max_goals_scored)
    print("Average Threshold goals scored: ",average_goals_scored)
    print ("Minimum Thrshold goals scored: ",min_goals_scored)
    print("\n")
    stats = league.compute_goals_scored_goals_conceded(teams,match_number)
    for st in stats:
        print("Scored "+str(st[1] )+" Conceded "+str(st[2]),st)

    grades = league.grade_all_teams(match_number) 
    
    print("Graded Teams: ")
    for grade in grades:
        print("Grade: ",grade,grades[grade])  

    both_to_score = league.both_teams_to_score(match_number)
    #for both teams to score.
    print("\nBoth Teams to score!!")
    for t in both_to_score:
        print("team",t)  
    print("\nBest Teams")
    best_teams = league.best_teams_to_win(match_number)
    for b in best_teams:
        print("Team: ",b) 

    print("\nsecond best teams: ")
    second_best = league.second_best_teams(match_number)
    for s in second_best:
        print("Team: ",s)  

    print("\nWorst teams: ")
    worst_teams = league.worst_teams(match_number)
    for w in worst_teams:
        print("Team: ",w)
    """
    ...
    grade_teams = menu(teams, league,match_number)
    print("\nGRADED TEAMS in LEAGUE.")
    print(grade_teams)
    """
